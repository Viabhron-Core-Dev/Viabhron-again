...
import { Moss, UIMode, Agent, CelestialChat } from '../types';
...
export function MossLoader({ 
  ...
  useEffect(() => {
    ...
    window.addEventListener('viabhron:close-all-moss', handleCloseAll);

    return () => {
      window.removeEventListener('viabhron:open-moss', handleOpenApp);
      window.removeEventListener('viabhron:close-all-moss', handleCloseAll);
    };
  }, [onAppOpen]);
