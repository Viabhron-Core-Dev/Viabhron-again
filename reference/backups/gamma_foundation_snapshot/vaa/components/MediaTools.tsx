...
import { Agent } from "@/core/kernel/types";
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.name === 'NotAllowedError' || err.message.includes('Permission denied')) {
          setError("Camera access denied. Please look for the camera icon in your browser's address bar. Also, try opening the app in a new tab if you are in a preview.");
        } else if (err.message.includes('Permission dismissed')) {
          setError("The permission request was dismissed. Please click 'Try Again' and grant access when the browser asks.");
...
export const QRScanner: React.FC<QRScannerProps> = ({ onClose, onScan }) => {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.name === 'NotAllowedError' || err.message.includes('Permission denied')) {
          setError("Camera access denied. Please reset permissions in your browser's address bar.");
        } else if (err.message.includes('Permission dismissed')) {
          setError("Request dismissed. Please click 'Try Again' and accept the prompt.");
        } else {
          setError(`Camera error: ${err.message}`);
        }
      } else {
