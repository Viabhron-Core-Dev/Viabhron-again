...
import { localDb } from "@/landscape/engines/db";
import { 
  User,
  Extension, 
  Secret, 
...
import { MachineRoom } from "@/landscape/modules/MachineRoom";
...
export const VaaClient: React.FC<VaaClientProps> = (props) => {
  const { 
    user, login, logout, agents = [], extensions = [], secrets = [], onCreateAgent, onAddSecret, 
    onDeleteSecret, onUpdateSecret, uiMode, onToggleUiMode,
    miniApps, clients, securityRules, efficiencyPatches, systemMode, isSidebarCollapsed, 
    setIsSidebarCollapsed, isSystemMenuOpen, setIsSystemMenuOpen, isTerminalOpen, setIsTerminalOpen, 
    isLockdown, setIsLockdown, tabs, activeTabId, setActiveTabId, handleAddTab, handleCloseTab, 
    handleShelveTab, handleWakeTab, onQuickAction, geminiApiKey, setGeminiApiKey, googleClientId, 
    setGoogleClientId, backgroundTasks, notifications, setNotifications, logs, addLog, 
    canvasViewMode, setCanvasViewMode, confirmationRequest, setConfirmationRequest, 
    bridgedProjectId, externalPlugins, setExternalPlugins, checkSovereignProcedures, 
    handleToggleMiniApp, handleToggleClient, handleToggleRule, handleTogglePatch, 
    handleConnectCloud, handleInstallExtension, handleProjectSelected, handleCodexRescue, 
    handleRatifyProposal, handleShelveProposal, handleVetoProposal, handleAddRule, 
    handleDeleteRule, handleLockdown, handleUnlock, handleModeChange, handleApplyPatch, 
    handleHatch, handleInstallPWA, handleDeleteAgent, handleAddAgent, isAddingAgent, 
    setIsAddingAgent, newAgentName, setNewAgentName, newAgentKey, setNewAgentKey, 
    accessToken, sops, proposals, setIsAgentSettingsOpen, isAgentSettingsOpen, 
    isTabSwitcherOpen, setIsTabSwitcherOpen, systemMenuRef, setSystemMode, officeName
  } = props;
  ...
  const renderTabContent = (tabId: string) => {
    switch (tabId) {
      ...
      case "loader":
        return <LoaderTab onOpenHatchery={() => setShowSovereignCheck(true)} onRefresh={() => toast.success("Matrix Synchronized")} />;
      default:
        return null;
    }
  };
  ...
  if (uiMode === 'browser' && view !== "workflow") {
    return (
      <MachineRoom 
        {...props}
        agents={agents || []}
    ...
    <div className="h-full flex flex-col bg-white relative overflow-hidden font-sans selection:bg-wa-header/10">
      ...
      <AnimatePresence mode="wait">
        {view === "workflow" ? (
          ...
          >
            {/* Header */}
            <div className="bg-wa-header text-white p-5 flex flex-col gap-1 shadow-lg z-20">
              <div className="flex justify-between items-center">
                <h1 
                  className="text-2xl font-black tracking-tight cursor-pointer hover:opacity-80 transition-opacity flex items-center gap-2"
                  onClick={() => {
                ...
                <div className="relative">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      if (activeTab === 'loader') setShowSovereignCheck(true);
                    }}
                  ...
                  >
                    ...
                    {activeTab === 'loader' && <Egg className="w-8 h-8" />}
                  </button>
            ...
            <div className="bg-white border-t border-slate-100 px-6 py-3 flex justify-between items-center z-40">
              {[
                { id: "chats", label: "CHATS", icon: <MessageCircle className="w-7 h-7" /> },
                { id: "news", label: "STATUS", icon: <RefreshCw className="w-7 h-7" /> },
                { id: "workflow", label: "WORKFLOW", icon: <WorkflowIcon className="w-7 h-7" /> },
                { id: "loader", label: "LOADER", icon: <Egg className="w-7 h-7" /> }
              ].map(tab => (
                <button 
                  key={tab.id}
