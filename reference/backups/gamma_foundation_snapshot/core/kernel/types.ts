...
export interface CanvasNode {
  id: string;
  type: 'ai_text' | 'ai_image' | 'github' | 'ui_header' | 'ui_sidebar' | 'ui_content' | 'input' | 'output' | 'agent' | 'logic';
...
export interface MCPSandbox {
  ...
  tokenEfficiency: number; // Percentage saved
  startTime: string;
}
...
export interface QuantumNode {
  id: string;
  name: string;
  provider: 'IonQ' | 'Rigetti' | 'D-Wave' | 'Custom';
